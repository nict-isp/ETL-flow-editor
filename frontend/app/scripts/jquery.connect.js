...
(function($) {
    $.connect = function(elem1, elem2, options) {
        ...
        var makeConnector = function(data) {
            ...
            var tableID = parent.options.container.split('#');
            var html = $("<div class='connector " + tableID[1] + "'><p class='left title'></p><p class='right title'></p></div>");
        ...
        var getCoords = function(elem, width, height) {
            function pX(elem) {
				//return elem.position() != undefined ? elem.position().left : false;
				return elem.position().left;
            }
            function pY(elem) {
                //return elem.position() != undefined ? elem.position().top : false;
